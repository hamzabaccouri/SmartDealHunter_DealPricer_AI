...
class App:
    ...
    def run(self):
        with gr.Blocks(title="The Price is Right", 
                      fill_width=True,
                      css=".custom-dataframe { max-height: 400px; overflow-y: auto; }") as ui:
            ...
            def do_select(selected_index: gr.SelectData):
                ...
                if self.agent_framework and self.agent_framework.planner and self.agent_framework.planner.messenger:
                    self.agent_framework.planner.messenger.alert(opportunity)
                else:
                    print("Warning: Agent framework or messenger not properly initialized")
